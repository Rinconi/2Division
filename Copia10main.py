...
if historial_file.exists():
    try:
        ...
        for k,v in j.items():
            if len(v) > len(PARTIDOS.get(k,[])):
                PARTIDOS[k] = [tuple(x) for x in v]
    except: pass
...
def fetch_ceuta():
    ...
    try:
        url = f"https://site.api.espn.com/apis/site/v2/sports/soccer/esp.2/scoreboard?dates={hoy}"
        r = requests.get(url, timeout=12, headers={"User-Agent":"Mozilla/5.0"})
        data = r.json()
        for ev in data.get("events",[]):
            if "ceuta" in ev.get("name","").lower():
                ...
                fecha = datetime.now(ZoneInfo("Europe/Madrid")).strftime("%Y-%m-%d")
                if any(fecha in x[0] for x in PARTIDOS["ceuta"]):
                    return False
                rc = "V" if int(home['score'])>int(away['score']) else "D" if int(home['score'])<int(away['score']) else "E"
                rf = "D" if rc=="V" else "V" if rc=="D" else "E"
                PARTIDOS["ceuta"].append((fecha,f"Ceuta {gol} Real Sociedad B",rc,"",gol))
                PARTIDOS["real-sociedad-b"].append((fecha,"",rf,f"Ceuta {gol} Real Sociedad B",gol))
                print(f"AUTO: Ceuta {gol} Real Sociedad B")
                return True
    except Exception as e:
        print(f"Auto pendiente: {e}")
    return False
...
story.append(Paragraph(f"Clasificacion EN VIVO - Jornada {J} - 26/27", style_sub))
